...
# permutation t test for paired and independent samples:
	...
	if(paired){
		# paired permutation t-test: corresponding pairs in x1 and x2 were randomly shuffled across the columns of cbind(x1, x2), and the mean differenced were computed.
		x = cbind(x1, x2)
		n.obs = dim(x)[1]
		for (i in 1:n){
	...
	test.out$p = sum(perm.stats < -abs(mean.diff) | perm.stats > abs(mean.diff))/n
...
#library(MASS)
# ginv()
feature.cv.test.pca = function(feature.in, factor, k = 10, method = "wilcox"){
	...
	inv.rotation = ginv(pca.out$rotation[, 1:10])
	feature.weight = feature.weight.pca[1:10] %*% inv.rotation

	return(feature.weight)
}

feature.cv.test = function(feature.in, factor, k, method="wilcox", glmnet.para, seed = 111){
# statistic tests to compute feature weights:
# if k == 1, run test on the whole sample without cross validation. the output is a vector with length same as the number of features i.e ncol(feature.in)
# this is useful in case we do boot strapping and compute coefficient of variation (CV) across boot strap samples.
	...
	for ( i in 1:k) {
		...
		if (method=="mean.diff"){
			...
			set.seed(444)
    		cv.fit = cv.glmnet(feature.in[cv.k!=i,], factor[cv.k!=i], nfold = glmnet.para$nfolds.inner, alpha = glmnet.para$alpha, family = glmnet.para$family, standardize = F)
...
feature.cv.boot = function(feature.in, factor, n=100, method, glmnet.para, pca = F){
...
glmnet.cv.fun = function(x, y, glmnet.para){
	...
	for (i in 1:k){	
		...
		if (penalty.weight == "none"){
			...
			if (penalty.weight == "mean.diff"){
				...
				f.weight = feature.cv.boot(x.train, y.train, n=500, method = "mean.diff", glmnet.para)
			}else if (penalty.weight == "pearson.boot"){
				f.weight = feature.cv.boot(x.train, y.train, n=500, method = "pearson", glmnet.para)
			}else if (penalty.weight == "biserial.boot"){
				f.weight = feature.cv.boot(x.train, y.train, n=500, method = "biserial", glmnet.para)
			}else if (penalty.weight == "glmnet.coef.boot"){
				f.weight = feature.cv.boot(x.train, y.train, n=500, method = "glmnet.coef", glmnet.para)
			}else if (penalty.weight == "mean.diff.boot.pca"){
				f.weight = feature.cv.boot(x.train, y.train, n=500, method = "mean.diff", glmnet.para, pca=T)
			}
			# compute coefficient of variation:
			f.weight.cv = scale.0.1(apply(f.weight, 2, sd)/apply(f.weight, 2, mean))
			# log transform the weights:
			
			if (glmnet.para$log.penalty.weight==1){
		...
		for (i.alpha in 1:length(alpha)) {
			...
			fit.list[[i.alpha]] = cv.glmnet(x.train, y.train, nfolds = glmnet.para$nfolds.inner, 
		...
		y.pred = predict(mod, x.test, s = lambda.min, type = glmnet.para$predict.type)
		y.pred.train = predict(mod, x.train, s = lambda.min, type = glmnet.para$predict.type)
	...
	result.coefs.ind = matrix(as.numeric(result.coefs!=0), nrow = nrow(result.coefs), ncol = ncol(result.coefs))[-1,]
	...
	result.coefs.ind = data.frame(colnames(x), result.coefs.ind, coefs.ind.sum, reproducibility.index, stringsAsFactors = F)

	glmnet.out = list()
	glmnet.out$test.result = test.result
	glmnet.out$train.result = train.result
	glmnet.out$coefs = result.coefs.ind
