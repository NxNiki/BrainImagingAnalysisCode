...
svm.cv.fun = function(subject.info, brain.feature, k, cost.seq, p.thresh){
	...
	for (i in 1:k){	
		...
		feature.selected = select.pc.feature(brain.feature.train, subject.info.train$ptsd, p.thresh)
		feature.train = cbind(subject.info.train, feature.selected)
		feature.test = cbind(subject.info.test, brain.feature.test)
