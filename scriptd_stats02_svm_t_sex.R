...
#brain.feature = scale(cbind(fc))
report.name = "sex_svmt_fslmultimodal"
...
select.feature = function(feature.in, factor, p){
	...
	print(colnames(feature.in)[feature.idx])
	return(feature.idx)
...
svm.cv.fun = function(subject.info, brain.feature, k, cost.seq, p.thresh){
	...
	idx.order.factor = order(subject.info$Sex)
	...
	for (i in 1:k){	
		...
		feature.idx = select.feature(brain.feature.train, subject.info.train$Sex, p.thresh)
		...
		svm.x.train = model.matrix(feature.train$Sex~., feature.train)
		svm.y.train = as.factor(feature.train$Sex)

		svm.x.test = model.matrix(feature.test$Sex~., feature.test)
		svm.y.test = as.factor(feature.test$Sex)
...
svm.cv.result = svm.cv.fun(subject.info[,-c(1,4)], brain.feature, k, cost.seq, p.thresh)
...
report = rbind(result, colMeans(result))
